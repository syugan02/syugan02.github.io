...
<img src="./img/레전드 오브 슬래셔 8주차 구현 영상.gif>

작업명 : 스테이지 기본 틀 제작, 플레이어 캐릭터 이동 및 공격, HP와 필살기 UI 제작, 적 슬라임 구현

해당 요구 사항 :

1) 모든 스테이지의 기초적인 틀이 되는, 기반이 되는 배경 제작. (꽃, 나무, 풀, 배경 오브젝트들 배치)

2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기.

3) 검을 무기로 하는 플레이어 캐릭터의 이동 속도, 체력, 대미지 수치 정하기.

4) 플레이어 캐릭터의 현재 체력을 알 수 있는 HP UI 구현하기.

5) 플레이어 캐릭터의 현재 필살기 게이지를 알 수 있는 필살기 UI 구현하기.

6) 적 슬라임의 이동, 공격, 피격 시스템 및 애니메이션 구현하기.

작업 내용 :

1) 파란 하늘과 구름, 해와 같은 오브젝트를 배치해 하늘 배경을 만들었고, 플레이어가 이동할 수 있는 땅과 땅만 있으면 밋밋하니 작은 꽃과 풀, 나무를 배치하였고, 플레이어 캐릭터의 이동 반경을 제한하기 위해 일정 거리에 벽을 세워두었다. 해당 벽은 눈으로 판별할 수 있게 현재는 보이는 상태이고 나중에 안 보이도록 막을 계획이다. >>> 진행도 60% (땅에 들판 텍스쳐를 입히고 오브젝트 배치가 맘에 안 들어서 수정할 계획)

2) 플레이어 캐릭터의 이동 시스템은 카메라가 바라보고 있는 시점을 기준으로 키보드를 누른 방향으로 움직이도록 구현하였다. 또 Q와 E키를 누르면 캐릭터와 카메라가 회전하도록 구현하였다. 약공격과 필살기 애니메이션은 구하였지만 제대로 된 시스템은 구현하지 못하였다. 약공격 시스템을 구현하는데 오류가 생겨 해당 부분을 고쳐야 한다. 필살기 시스템은 아직 손을 대보지 못하였다. >>> 진행도 40% (이동 구현은 완료, 약공격 오류 수정 필요, 필살기 시스템 구현 및 테스트 필요)

3) 플레이어 캐릭터의 이동 속도는 5, 체력은 100, 약공격의 대미지 수치는 아직 정하지 못하였다. >>> 진행도 80% (후에 이 수치들은 밸런스를 위해 바뀔 수 있음)

4) 게임 화면 좌측 하단에 빨간색으로 되어있는 HP바를 만들었다. 현재 체력에 따라 UI가 변화하며 디자인은 시간적 여유가 될 때 수정할 예정이다. >>> 진행도 95% (이후 여유가 생길 때 디자인 수정하기)

5) 게임 화면 우측 하단에 현재 필살기 게이지를 알 수 있는 필살기 게이지를 만들었다. 필살기 게이지에 따라 UI가 변화한다. 디자인은 시간적 여유가 될 때 수정할 예정이다. >>> 진행도 80% (이후 여유가 생길 때 UI디자인을 처음 생각한 디자인으로 수정할 예정)

6) 적 슬라임 구현은 위에 플레이어 구현할 때 예정과 다르게 시간을 많이 소요해서 스크립트에 이동 속도와 체력, 대미지만 적고 구현한 시간이 없다. 다음 작업 때 우선적으로 슬라임을 구현할 예정 >>> 진행도 7% (이동 속도, 체력, 대미지만 적어두고 구현한 것이 없음.)
