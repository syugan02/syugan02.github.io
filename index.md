...
|그림|이름|근/원거리|특이사항|
|:---:|:---:|:---:|:---:|
| <img src="./img/슬라임.png" width = 100 heigth = 100> |슬라임|근거리|없음|
| <img src="./img/고블린.png" width = 100 heigth = 100> |고블린|근거리|없음|
| <img src="./img/활 고블린.png" width = 100 heigth = 100> |활 고블린|원거리|플레이어 위치에 화살을 쏴 공격|
| <img src="./img/오크.png" width = 100 heigth = 100> |오크|원거리|플레이어 위치에 마법진을 보여준 후 지속 공격|
| <img src="./img/골렘.png" width = 100 heigth = 100> |골렘|근/원거리|네임드 적, 근접 공격과 멀리 떨어지면 원거리 공격을 하며 원거리 공격에 맞으면 스턴 상태가 됨|
| <img src="./img/스켈레톤.png" width = 100 heigth = 100> |스켈레톤|근거리|없음|
| <img src="./img/드래곤.png" width = 100 heigth = 200> |드래곤|근/원거리|보스, 근접 공격과 멀리 떨어지면 원거리 공격을 하며, 굉장히 멀리 떨어지면 플레이어를 향해 날아옴|
...
|그림|이름|설명|
|:---:|:---:|:---:|
| <img src="./img/체력 회복 포션.png" width = 100 heigth = 100> |체력 회복 포션|사용하면 체력을 일정량 회복시켜주는 포션. 1을 눌러 사용이 가능하다.|
| <img src="./img/SP 회복 포션.png" width = 100 heigth = 100> |SP 회복 포션|사용하면 필살기 게이지를 일정량 회복시켜주는 포션. 2를 눌러 사용이 가능하다.|
| <img src="./img/폭탄.png" width = 100 heigth = 100> |폭탄|사용하면 지정 범위로 날아가 대미지와 스턴을 주는 아이템. F키를 눌러 마우스를 움직이면 위치를 설정하고, F키를 떼면 해당 위치로 날아간다.|
...
<br>
1.1) 모든 스테이지의 기초적인 틀이 되는, 기반이 되는 배경 제작. (꽃, 나무, 풀, 배경 오브젝트들 배치) (4)  <br>

1.2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. (6)<br>

1.3) 검을 무기로 하는 캐릭터의 이동 속도, 체력, 대미지 수치 정하기. (7)<br/>

1.4) 플레이어 캐릭터의 현재 체력을 알 수 있는 HP UI 구현하기. (19)<br/>

1.5) 화면 우측 하단에는 필살기 사용 여부를 알 수 있는 필살기 게이지 UI가 있다. (25)<br/>

1.6) 적 슬라임의 이동, 공격, 피격, 사망 시스템 및 애니메이션 구현하기. (41)<br/><br><br>



2.1) 적 슬라임의 이동속도, 대미지, 체력 수치 정하기 (42)<br/>

2.2) 적 고블린의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. (43)<br/>

2.3) 적 고블린의 이동속도, 대미지, 체력 수치 정하기 (44)<br/>

2.4) 적의 종류를 지정해서 지정된 적이 랜덤하게 생성되는 게임 매니저 오브젝트 구현(적의 생성되는 수는 최대치가 정해져 있음) (12)<br/><br><br>



3.1) 적의 공격을 맞으면 각 적 대미지 수치에 따라 플레이어의 HP가 변화하도록 구현 (21)<br/>

3.2) 플레이어 HP UI 오른쪽에는 아이템 박스 UI가 존재하여 아이템 소지 여부를 알 수 있다. (20)<br/>

3.3) 체력 회복 물약의 아이템 정보 및 체력 회복 수치 구현. (30)<br/>

3.4) 적의 HP가 0이 되면 해당 적은 사라진다. 그리고 0이 됐을 경우 일정 확률로 체력 회복 포션을 드랍한다. (31)<br/>

3.5) 플레이어가 아이템에 다가가면 아이템이 자동 습득되어 아이템 박스 UI 아이템 이미지를 표시하도록 구현. 만약 아이템을 이미 소지하고 있을 경우에 접촉할 경우 필드에 있는 아이템은 그냥 사라지도록 구현. (32)<br/>

3.6) 아이템을 소지하고 있을 경우 Space바를 누르면 플레이어의 체력이 일정치 회복되도록 구현. (33)<br/><br><br>



4.1) 게임 매니저 오브젝트에 각 클리어 목표와 현재 상태를 비교하는 시스템 추가 구현 (13)<br/>

4.2) 화면 우측에는 클리어 조건과 현재 상태를 알려주는 UI가 있다. (23)<br/>

4.3) 화면 좌측에는 플레이어가 잡은 적의 수를 표시해주는 UI가 존재한다. (18)<br/>

4.4) 적을 일정 수 이상 잡았을 경우 게임 매니저 오브젝트에 할당되어 있는 효과음이 재생되도록 구현 (15)

4.5) 필살기 게이지는 적을 일정 수량 잡으면 게임 매니저 오브젝트에서 이를 판정해서 플레이어에게 일정량의 필살기 게이지를 충전시켜준다. (26)<br/><br><br>



5.1) 적에게 공격이 명중됐을 경우 필살기 게이지가 일정량 회복되는 시스템을 플레이어에게 추가한다. 해당 시스템은 내부 쿨타임이 존재한다. (27)<br/>

5.2) 화면 우측 상단에는 현재 자신이 바라보고 있는 방향과 적들의 위치를 간략하게 알 수 있는 미니맵 UI가 있다. (29)<br/>

5.3) 적 오크의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. (45)<br/>

5.4) 적 오크의 이동속도, 대미지, 체력 수치 정하기 (46)<br/><br><br>



6.1) 스테이지가 클리어 되면 클리어 문구 UI가 표시된다. (59)<br/>

6.2) 스테이지 중 ESC 버튼을 누르면 게임이 일시정지가 되며 일시정지 UI가 표시된다 (40)<br/>

6.3) 검을 무기로 하는 캐릭터의 공격 명중 이펙트와 효과음을 구현한다. (8)<br/>

6.4) 1 스테이지는 약간의 튜토리얼 형식으로 리스폰되는 적은 슬라임, 고블린, 오크로 3종류다. 생성되는 적을 전부 잡으면 클리어 된다. (53)<br/>
</details>
...
<br>
작업명 : 스테이지 기본 틀 제작, 플레이어 캐릭터 이동 및 공격, HP와 필살기 UI 제작, 적 슬라임 구현<br/><br>

해당 요구사항 : <br/><br>

1) 모든 스테이지의 기초적인 틀이 되는, 기반이 되는 배경 제작. (꽃, 나무, 풀, 배경 오브젝트들 배치) **진행도 60%**<br/>

2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. **진행도 40%**<br/><br>

3) 검을 무기로 하는 플레이어 캐릭터의 이동 속도, 체력, 대미지 수치 정하기. **진행도 80%**<br/><br>

4) 플레이어 캐릭터의 현재 체력을 알 수 있는 HP UI 구현하기. **진행도 95%**<br/><br>

5) 플레이어 캐릭터의 현재 필살기 게이지를 알 수 있는 필살기 UI 구현하기. **진행도 80%**<br/><br>

6) 적 슬라임의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. **진행도 7%**<br/><br><br>


작업 내용 : <br/><br>
1) 모든 스테이지의 기초적인 틀이 되는, 기반이 되는 배경 제작. (꽃, 나무, 풀, 배경 오브젝트들 배치) **진행도 60%**(땅에 들판 텍스쳐 입히고 오브젝트 배치 수정)<br/><br>

2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. **진행도 40%**(이동 구현은 완료, 약공격 오류 수정 필요, 필살기 시스템 구현 및 테스트 필요)<br/><br>

3) 검을 무기로 하는 플레이어 캐릭터의 이동 속도, 체력, 대미지 수치 정하기. **진행도 80%**(대미지 수치 정하기, 후에 이 수치들은 밸런스를 위해 바뀔 수 있음)<br/><br>

4) 플레이어 캐릭터의 현재 체력을 알 수 있는 HP UI 구현하기. **진행도 95%**(이후 여유가 생길 때 디자인 수정하기)<br/><br>

5) 플레이어 캐릭터의 현재 필살기 게이지를 알 수 있는 필살기 UI 구현하기. **진행도 80%**(이후 여유가 생길 때 UI디자인을 처음 생각한 디자인으로 수정)<br/><br>

6) 적 슬라임의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. **진행도 7%**(이동 속도, 체력, 대미지 수치만 정함.)<br/>
</details>
...
<br>
작업명 : 플레이어 캐릭터 시스템 구현 및 애니메이션, 적 슬라임, 고블린 구현, 게임 매니저 오브젝트 구현<br/><br>

해당 요구사항 :<br/><br>

1.2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. (6) **진행도 40%**<br/><br>

1.6) 적 슬라임의 이동, 공격, 피격, 사망 시스템 및 애니메이션 구현하기. (41) **진행도 50%**<br/><br>

2.1) 적 슬라임의 이동속도, 대미지, 체력 수치 정하기 (42) **진행도 100%**<br/><br>

2.6) 적의 종류를 지정해서 지정된 적이 랜덤하게 생성되는 게임 매니저 오브젝트 구현(적의 생성되는 수는 최대치가 정해져 있음) (12) **진행도 100%**<br/><br>

2.2) 적 고블린의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. (43) **진행도 50%**<br/><br>

2.3) 적 고블린의 이동속도, 대미지, 체력 수치 정하기 (44) **진행도 100%**<br/><br><br>


작업 내용 :<br/><br>

적 고블린의 수치는 슬라임 보다 강하게 측정하기<br/><br>

게임 매니저 오브젝트를 만들어서 스테이지 둔 후 게임 오브젝트를 리스트로 받아 확률적으로 적이 생성되도록 만들기<br/><br>

적의 리스폰 범위는 스크립트에서 좌표 범위를 받아 정해진 범위 내에서 랜덤하게 생성되게 만든다.<br/><br>
</details>
...
<br>
작업명 : 플레이어 시스템 및 애니메이션 수정, 적 시스템 및 애니메이션 구현, 체력 회복 아이템 구현 및 아이템 박스 UI 제작<br/><br>

해당 요구사항 : <br/><br>

1.2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. (6) **진행도 50%** (약공격 시스템 및 애니메이션은 구현, 하지만 자연스럽지 않아 수정 필요)<br/><br>

1.6) 적 슬라임의 이동, 공격, 피격, 사망 시스템 및 애니메이션 구현하기. (41) **진행도 50%**<br/><br>

2.2) 적 고블린의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. (43) **진행도 50%**<br/><br>

3.1) 적의 공격을 맞으면 각 적 대미지 수치에 따라 플레이어의 HP가 변화하도록 구현 (21) **진행도 0%**<br/><br>

3.2) 플레이어 HP UI 오른쪽에는 아이템 박스 UI가 존재하여 아이템 소지 여부를 알 수 있다. (20) **진행도 100%**<br/><br>

3.3) 체력 회복 물약의 아이템 정보 및 체력 회복 수치 구현. (30) **진행도 100%**<br/><br>

3.4) 적의 HP가 0이 되면 해당 적은 사라진다. 그리고 0이 됐을 경우 일정 확률로 체력 회복 포션을 드랍한다. (31) **진행도 100%**<br/><br>

3.5) 플레이어가 아이템에 다가가면 아이템이 자동 습득되어 아이템 박스 UI 아이템 이미지를 표시하도록 구현. 만약 아이템을 이미 소지하고 있을 경우에 접촉할 경우 필드에 있는 아이템은 그냥 사라지도록 구현. (32) **진행도 100%**<br/><br>

3.6) 아이템을 소지하고 있을 경우 Space바를 누르면 플레이어의 체력이 일정치 회복되도록 구현. (33) **진행도 100%**<br/><br><br>

작업 내용 : <br/><br>

HP UI 옆에 아이템 박스 UI 구현해 아이템을 얻을 경우 해당 UI 이미지 보이기<br/><br>

체력 회복 포션의 아이템 정보(회복량) 구현<br/><br>
</details>
...
<br>
작업명 : 플레이어 구현, 게임 매니저 오브젝트 킬 수 및 현재 목표 관리, 적의 공격 피격 시스템 구현<br/><br>

해당 요구사항 : <br/><br>

1.2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. (6) **진행도 60%**<br/><br>

1.6) 적 슬라임의 이동, 공격, 피격, 사망 시스템 및 애니메이션 구현하기. (41) **진행도 80%** (플레이어의 공격을 맞으면 HP가 닳도록 구현)<br/><br>

2.2) 적 고블린의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. (43) **진행도 80%** (플레이어의 공격을 맞으면 HP가 닳도록 구현)<br/><br>

3.1) 적의 공격을 맞으면 각 적 대미지 수치에 따라 플레이어의 HP가 변화하도록 구현 (21) **진행도 100%**<br/><br>

4.1) 게임 매니저 오브젝트에 각 클리어 목표와 현재 상태를 비교하는 시스템 추가 구현 (13) **진행도 100%**<br/><br>

4.2) 화면 우측에는 클리어 조건과 현재 상태를 알려주는 UI가 있다. (23) **진행도 100%**<br/><br>

4.3) 화면 좌측에는 플레이어가 잡은 적의 수를 표시해주는 UI가 존재한다. (18) **진행도 100%**<br/><br>

4.4) 적을 일정 수 이상 잡았을 경우 게임 매니저 오브젝트에 할당되어 있는 효과음이 재생되도록 구현 (15) **진행도 100%**<br/><br>

4.5) 필살기 게이지는 적을 일정 수량 잡으면 게임 매니저 오브젝트에서 이를 판정해서 플레이어에게 일정량의 필살기 게이지를 충전시켜준다. (26) **진행도 100%**<br/><br><br>

작업 내용 : <br/><br>

게임 매니저 오브젝트에 스테이지 클리어 목표와 현재 목표를 체크하는 코딩<br/><br>

적이 사망 시 게임 매니저 오브젝트에서 킬 수 관리가 1스택이 증가하는 코딩<br/><br>

킬 수가 일정 수를 넘으면 효과음 재생과 플레이어 필살기 게이지 회복<br/><br>
</details>
...
<br>
작업명 : 검 플레이어 캐릭터 오류 수정, 필살기 충전 시스템 구현, 미니맵 구현, 적 종류 오크 구현<br/><br>

해당 요구사항 : <br/><br>

1.2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. (6) **진행도 75%** (필살기 공격 코드 작성 완료, 약공격 시스템 좀 더 다듬어야 함)<br/><br>

5.1) 적에게 공격이 명중됐을 경우 필살기 게이지가 일정량 회복되는 시스템을 플레이어에게 추가한다. 해당 시스템은 내부 쿨타임이 존재한다. (27) **진행도 100%**<br/><br>

5.2) 화면 우측 상단에는 현재 자신이 바라보고 있는 방향과 적들의 위치를 간략하게 알 수 있는 미니맵 UI가 있다. (29) **진행도 100%** (원하는 형식으로 구현은 됐고 디자인 부분만 수정하기)<br/><br>

5.3) 적 오크의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. (45) **진행도 100%**<br/><br>

5.4) 적 오크의 이동속도, 대미지, 체력 수치 정하기 (46) **진행도 100%**<br/><br><br>

작업 내용 : <br/><br>

플레이어 캐릭터 안에 내부 쿨타임을 설정해서 공격이 명중했을 경우 필살기 UI가 채워지도록 구현<br/><br>

미니맵 UI 구현<br/><br>

적 오크의 시스템을 구현하면서 게임 스테이지 매니저 오브젝트에서 오크가 생성되도록 추가 구현<br/><br>
...
<br>
작업명 : 플레이어 공격 모션 수정 및 다듬기, 스테이지 클리어 UI 및 일시정지 UI 제작, 게임 1스테이지 제작<br/><br>

해당 요구사항 : <br/><br>

1.2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. (6) **진행도 90%** (필살기 이펙트 구현, 필살기 공격 구현)<br/><br>

6.1) 스테이지가 클리어 되면 클리어 문구 UI가 표시된다. (59) **진행도 20%** (UI창과 코드만 작성했고 실제로 실행되는지 확인 못 함)<br/><br>

6.2) 스테이지 중 ESC 버튼을 누르면 게임이 일시정지가 되며 일시정지 UI가 표시된다 (40) **진행도 100%**<br/><br>

6.3) 검을 무기로 하는 캐릭터의 공격 명중 이펙트와 효과음을 구현한다. (8) **진행도 30%** (공격 명중 시 이펙트만 나오게 만듦. 이펙트 위치 수정 및 효과음 추가 필요)<br/><br>

6.4) 1 스테이지는 약간의 튜토리얼 형식으로 리스폰되는 적은 슬라임, 고블린, 오크로 3종류다. 생성되는 적을 전부 잡으면 클리어 된다. (53) **진행도 75%** (오크가 나오는지 확인 및 각 적들 종류 배치 고려, 전부 잡았을 경우 클리어 판정이 나는지 확인 필요)<br/><br><br>

작업 내용 : <br/><br>

게임 매니저 오브젝트에 클리어 목표를 달성하였는지 비교하는 코드를 작성하고 달성했을 경우 클리어 UI가 출력된다<br/><br>

게임 중 ESC를 누르면 게임이 일시정지 되도록 구현<br/><br>

공격이 적에게 명중했을 경우 공격 명중 이펙트와 효과음을 표시한다<br/><br>
...
1. 현 게임 조작 버튼을 마우스도 활용하여 게임을 진행하도록 변경<br/>
1-1. 마우스 이동-카메라 회전, 왼쪽 클릭-일반 공격<br/><br>

2. 강공격 모션 3가지와 필살기 추가 모션 1가지를 애니메이션 에셋을 추가하여 유니티 내에서 정상 동작 확인<br/>
</details>
...
1. 플레이어 강공격 시스템 구현<br/>
1-1. 마우스 오른쪽 버튼을 누를 시 강공격 실행<br/>
1-2. 강공격 모션은 일반 공격 타수마다 다르게 나오도록 구현<br/>
1-3. 강공격 대미지 및 범위를 따로 만들어 일반 공격과 다르게 적용 되도록 만듦<br/><br>

2. 적의 피격 애니메이션 구현<br/>
2-1. 플레이어에게 공격을 맞으면 스택이 쌓이며 스택이 일정치 이상이 될 경우 피격 애니메이션이 재생<br/><br>

3. 적들의 체력바 구현 및 플레이어 공격 범위 안에 있는 경우에만 표시되도록 구현<br/>
3-1. 영역 밖인데 체력바가 표시되는 버그가 발생, 다음 주차에 원인을 알고 해결하도록 함.<br/><br>

4. 기존 일반 공격 애니메이션 연결 방식을 개선<br/>
4-1. 해당 작업으로 일반 공격이 끊기는 현상이 조금 조금 완화됨<br/><br>

5. 공격 이펙트 발생 시간 개선<br/>
5-1. 해당 작업은 완료하지 못함. 일반 공격 방식이 바뀜으로 기존 이펙트 발생 방식을 고쳐서 현재 방식에 맞출 예정<br/>
</details>
...
1. 적들 체력바가 범위를 벗어나도 유지되던 버그 수정<br/><br>

2. 플레이어 회피 버튼 추가<br/>
2-1. 왼쪽 시프트 키를 눌러서 회피가 가능<br/>
2-2. 피격과 필살기 상태를제외한 모든 상태에서 사용이 가능<br/>
2-3. 회피 상태에서는 무적시간을 갖고 있으며 최대 2번 연속 사용이 가능하고 그 다음에는 쿨타임이 존재<br/><br>

3. 플레이어 필살기 종류와 모션을 하나 더 추가<br/>
3-1. 필살기 종류는 총 두 개로 둘 중 하나가 랜덤으로 나가는 형식으로 구현<br/>
3-2. 두 개의 필살기는 이펙트와 모션이 서로 다름<br/><br>

4. 공격 이펙트 및 효과음을 바뀐 일반 공격 방식에 맞게 발생하도록 변경<br/><br>

5. 적 피격 시스템 방식 변경<br/>
5-1. 피격 스택 시스템을 없애고, 캐릭터의 공격을 맞으면 무조건 피격 애니메이션이 재생되도록 변경<br/><br>

6. 원거리 종류 적 2마리 에셋 자료 확보<br/>
6-1. 하나는 멀리서 활을 쏘는 원거리 적<br/>
6-2. 다른 하나는 마법을 써서 일정 범위에 지속 대미지를 주는 원거리 적<br/>
6-3. 인게임 구현은 하지 않음<br/>
</details>
...
1. 원거리 적 활 고블린 구현<br/>
1-1. 맵 상에 소환되면 플레이어 위치를 향해 화살을 쏨<br/>
1-2. 플레이어와 거리가 가까워지면 다른 위치로 이동하여 거리를 벌린 다음 다시 화살을 쏨<br/><br>

2. 1 스테이지 적 종류와 적의 숫자 배치하기<br/>
2-1. 적은 슬라임, 고블린, 활 고블린 총 3종류로 구성되어 있음.<br/>
2-2. 총 적이 30마리가 나오며 30마리를 다 잡으면 해당 스테이지는 클리어가 됨.<br/>
2-3. 테스트 플레이를 해 본 결과 미숙한 부분이 많이 느껴짐<br/>
</details>
...
1. 원거리 적 오크 주술사 구현<br/>
1-1. 현 플레이어 위치에 마법진을 보여준 다음 일정 시간이 지나면 마법진 구역에 마법 효과가 나와 범위 안에서는 대미지를 지속적으로 입음<br/><br>

2. 인게임에서 적들이 제대로 동작하는지 확인<br/><br>

3. 적들의 대미지 수치 및 이동속도, 체력 수치 조정<br/><br>

4. 각 스테이지의 나오는 적의 종류와 수량 조절<br/><br>

5. 게임 오버 시 게임 재시작, 타이틀 화면으로 버튼 구현<br/><br>

6. 적의 공격 방식, 플레이어를 향한 추격 시스템 개선<br/>
6-1. 적이 플레이어에게 다가오면 동작 3종류(가만히 있기, 뒷걸음질, 공격) 이 중 하나를 실행하도록 만듦<br/><br>

7. 플레이어 스킬 및 인게임 UI 추가<br/>
7-1. R키를 눌러 사용 가능<br/>
7-2. 플레이어 중심으로 일정 범위 안의 적들을 공격하며 공격에 맞은 적들은 뒤로 물러나고 일정 시간 동안 스턴 상태에 빠짐<br/>
7-3. UI는 필살기 게잉지 왼쪽에 위치하며, 스킬을 사용하면 쿨타임이 표시됨<br/>
</details>
...
1. 4 스테이지 한정 오브젝트 텐트 추가<br/>
1-1. 텐트는 고유의 체력이 존재하며 체력이 0이 될 경우 게임 오버가 됨<br/>
1-2. 텐트는 스테이지 정중앙에 위치하며 움직이지 않음<br/>
1-3. 텐트의 체력은 지속적으로 볼 수 있도록 상단 좌측에 텐트 체력 UI를 추가<br/><br>

2. 4 스테이지 한정 적들의 움직임 바꾸기<br/>
2-1. 적들이 소환되면 텐트를 우선적으로 공격함<br/>
2-2. 플레이어의 공격을 받을 경우 공격 목표는 플레이어로 바뀜<br/>
2-3. 소환 장소는 맵 어디서나 소환되던 기존 스테이지와 다르게 동, 서, 남, 북 4곳에서만 나옴<br/>
</details>
...
1. 게임 자동 세이브 기능과 세이브 불러오기 기능 추가<br/>
1-1. 자동 세이브는 스테이지를 클리어했을 경우 진행도가 자동으로 세이브 됨<br/><br>

2. SP 회복 포션 아이템 추가, 프리펩과 스크립트 작성<br/>
2-1. 인게임 구현 및 테스트는 아직 안 해봄<br/>
2-2. 사용할 경우 필살기 게이지가 일정량 회복됨<br/><br>

3. 투척 아이템 폭탄 던지는 시스템 구현<br/>
3-1. 아이템을 획득하고 F키를 눌러 던질 위치를 정하고 F키를 떼서 던짐<br/>
3-2. F키를 누르는 동안 빨강색 원으로 어느 위치에 폭발 범위가 생기는지 보여줌<br/>
3-3. 현재 폭탄의 모델링은 따로 없으며 지정된 구역으로 잘 날아가는지 테스트만 해봄<br/><br>

4. 아이템 UI 변경<br/>
4-1. SP 회복 포션과 폭탄 아이템이 표시되는 UI 구현<br/>
4-2. 체력 회복 포션과 SP 회복 포션 UI 위치를 체력바 위로 변경<br/>
4-3. 폭탄 아이템 UI 위치를 스킬 아이콘 왼편에 보이도록 구현<br/>
</details>
...
1. 투척 아이템 폭탄의 모델링 및 터지는 이펙트 추가<br/>
1-1. 모델링을 폭탄으로 변경<br/>
1-2. 던진 후 땅에 닿으면 일정 범위에 폭발 이펙트와 공격 판정이 생김<br/>
1-3. 적이 폭탄에 맞은 경우 대미지를 입고 스턴 상태가 됨<br/><br>

2. 투척 시 폭발 범위를 알려주는 외관 변경<br/>
2-1. 빨강색 원에서 조준점으로 변경<br/><br>

3. 인게임 아이템 박스 및 체력바 디자인 변경<br/><br>

4. 각종 효과음 추가<br/>
4-1. 플레이어 : 스킬 발동 효과음, 폭탄을 던질 때 효과음, 폭탄 터질 때 효과음<br/>
4-2. 슬라임 : 공격 시 효과음<br/>
4-3. 활 고블린 : 공격 시 효과음<br/>
4-4. 오크 : 공격 시 효과음, 메테오 효과음<br/>
4-5. 스켈레톤 : 사망 시 효과음<br/>
</details>
...
1. UI 디자인 다듬기<br/>
1-1. 게임 오버와 게임 일시정지, 스테이지 클리어, 타이틀 화면의 UI 디자인을 게임 배경과 알맞게 디자인 변경<br/>
1-2. 폰트를 게임 전체적인 컨셉 디자인과 알맞도록 변경<br/><br>

2. 각종 효과음 추가<br/><br>

3. 각 적들과 1:1 상황을 만들어 효과음과 애니메이션 싱크 확인 및 버그 수정<br/><br>

4. 최종보스 드래곤 공격 패턴 구현<br/>
4-1. 플레이어와 멀리 떨어져 있을 경우 플레이어 위치를 향해 원거리 공격, 파이어볼을 3방향으로 발사<br/>
4-2. HP가 일정치 소모되었을 경우 지정된 위치 4곳 중 랜덤한 위치로 이동한 후 원거리 공격<br/>
</details>
...
1. 드래곤 각종 수정<br/>
1-1. 드래곤이 위치를 변경할 때 상승-이동-착륙의 순서를 거치도록 변경<br/>
1-2. 드래곤 원거리 공격 파이어볼 외형 디자인 수정<br/>
1-3. 드래곤이 출현했을 경우 화면 상단에 체력이 표시되도록 UI 수정<br/><br>

2. 적 골렘 공격 방식 구현<br/>
2-1. 플레이어와 멀리 떨어져 있을 경우 플레이어 위치에 돌을 던지는 원거리 공격을 하며, 플레이어와의 거리를 좁힘<br/>
2-2. 플레이어와 가까이 있을 경우 근접 공격 실행<br/>
</details>
...
1. 드래곤 행동 방식 수정<br/>
1-1. 플레이어를 계속 추적하도록 변경<br/>
1-2. 일정 거리 이상 멀어질 경우 파이어볼 원거리 공격 실행<br/>
1-3. 더욱 멀어질 경우 공중으로 날아서 플레이어에게 다가옴<br/><br>

2. 파이어볼과 골렘의 바위에 맞았을 경우 피격 이펙트 추가<br/><br>

3. 드래곤 HP바 UI 디자인 수정<br/><br>
...
1. 조작 설명 UI 버튼을 누르면 각 설명이 갱신되도록 구현<br/>
1-1. 각 버튼에 대한 설명 텍스트와 이미지가 갱신되도록 구현<br/>
1-2. 설명에 맞는 이미지는 아직 미설정<br/><br>

2. 타이틀 화면 이미지와 이펙트 추가<br/><br>

3. 인게임 배경 추가<br/><br>

4. 골렘 원거리 공격 바위와 폭탄을 던질 때 오브젝트들이 회전하도록 변경<br/><br>

5. 골렘 원거리 공격에 맞을 경우 일정시간 움직일 수 없는 스턴 추가<br/><br>

6. 적 드래곤이 나왔을 경우 포효와 함께 카메라 흔들림 효과 추가<br/><br>

7. 적 드래곤 착지 시 카메라 흔들림 효과 추가<br/><br>

8. 적 드래곤 착지 공격에 맞을 경우 일정시간 움직일 수 없는 스턴 추가<br/><br>

9. 적 드래곤 효과음 구현 및 애니메이션에 맞도록 싱크 조절<br/><br>

10. 각 스테이지 BGM이 다르게 나오도록 구현<br/><br>

11. 게임 처음 시작 및 각 스테이지 시작 시 로딩 화면이 나오도록 구현<br/>
11-1. 스테이지를 클리어할수록 화면에 캐릭터가 추가됨<br/><br>

12. 세이브 데이터가 있는 상태에서 게임을 처음부터 시작할 경우 기존 세이브 데이터가 지워지도록 구현<br/>
12-1. 시작하기 전에 세이브 데이터가 지워진다는 알림 창이 보이도록 구현<br/>
</details>
...
1. 플레이어 체력이 0이 될 경우 쓰러지는 애니메이션 추가<br/><br>

2. 모든 스테이지를 클리어했을 경우 간단한 텍스트와 컷신과 함께 엔딩으로 넘어가도록 구현<br/><br>

3. 게임 파일 빌드 후 테스트 플레이 및 오류 수정<br/>
3-1. 해당 과정 중 로딩화면에서 로딩이 멈추는 오류 발견, 로딩화면은 삭제<br/>
3-2. 게임을 좀 더 쉽게 즐길 수 있도록 아이템 드랍 확률 조정<br/>
3-3. 게임을 좀 더 쉽게 즐길 수 있도록 적들의 체력 변경<br/><br>

4. 1분 게임 홍보 영상 제작<br/><br>
