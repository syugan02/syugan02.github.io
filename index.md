...
1.1) 모든 스테이지의 기초적인 틀이 되는, 기반이 되는 배경 제작. (꽃, 나무, 풀, 배경 오브젝트들 배치) (4)  <br><br>

1.2) 검을 무기로 하는 플레이어 캐릭터의 약공격, 필살기, 이동 시스템 구현 및 애니메이션 구현하기. (6)<br><br>

1.3) 검을 무기로 하는 캐릭터의 이동 속도, 체력, 대미지 수치 정하기. (7)<br/><br>

1.4) 플레이어 캐릭터의 현재 체력을 알 수 있는 HP UI 구현하기. (19)<br/><br>

1.5) 화면 우측 하단에는 필살기 사용 여부를 알 수 있는 필살기 게이지 UI가 있다. (25)<br/><br>

1.6) 적 슬라임의 이동, 공격, 피격, 사망 시스템 및 애니메이션 구현하기. (41)<br/><br><br>



2.1) 적 슬라임의 이동속도, 대미지, 체력 수치 정하기 (42)<br/><br>

2.2) 적 고블린의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. (43)<br/><br>

2.3) 적 고블린의 이동속도, 대미지, 체력 수치 정하기 (44)<br/><br>
...
작업명 : 스테이지 기본 틀 제작, 플레이어 캐릭터 이동 및 공격, HP와 필살기 UI 제작, 적 슬라임 구현<br/>
...
6) 적 슬라임의 이동, 공격, 피격 시스템 및 애니메이션 구현하기. **진행도 7%**<br/>
...
작업명 : 플레이어 캐릭터 시스템 구현 및 애니메이션, 적 슬라임, 고블린 구현, 게임 매니저 오브젝트 구현<br/>
...
2.3) 적 고블린의 이동속도, 대미지, 체력 수치 정하기 (44) **진행도 100%**<br/>
...
작업명 : 플레이어 시스템 및 애니메이션 수정, 적 시스템 및 애니메이션 구현, 체력 회복 아이템 구현 및 아이템 박스 UI 제작<br/>
...
3.6) 아이템을 소지하고 있을 경우 Space바를 누르면 플레이어의 체력이 일정치 회복되도록 구현. (33) **진행도 100%**<br/>
...
작업명 : 플레이어 구현, 게임 매니저 오브젝트 킬 수 및 현재 목표 관리, 적의 공격 피격 시스템 구현<br/>
...
4.5) 필살기 게이지는 적을 일정 수량 잡으면 게임 매니저 오브젝트에서 이를 판정해서 플레이어에게 일정량의 필살기 게이지를 충전시켜준다. (26) **진행도 100%**<br/>
...
작업명 : 검 플레이어 캐릭터 오류 수정, 필살기 충전 시스템 구현, 미니맵 구현, 적 종류 오크 구현<br/>
...
5.4) 적 오크의 이동속도, 대미지, 체력 수치 정하기 (46) **진행도 100%**<br/>
...
작업명 : 플레이어 공격 모션 수정 및 다듬기, 스테이지 클리어 UI 및 일시정지 UI 제작, 게임 1스테이지 제작<br/>
...
6.4) 1 스테이지는 약간의 튜토리얼 형식으로 리스폰되는 적은 슬라임, 고블린, 오크로 3종류다. 생성되는 적을 전부 잡으면 클리어 된다. (53) **진행도 75%** (오크가 나오는지 확인 및 각 적들 종류 배치 고려, 전부 잡았을 경우 클리어 판정이 나는지 확인 필요)<br/>
...
1-1. 마우스 이동-카메라 회전, 왼쪽 클릭-일반 공격<br/>
...
1-3. 강공격 대미지 및 범위를 따로 만들어 일반 공격과 다르게 적용 되도록 만듦<br/>

2. 적의 피격 애니메이션 구현<br/>
2-1. 플레이어에게 공격을 맞으면 스택이 쌓이며 스택이 일정치 이상이 될 경우 피격 애니메이션이 재생<br/>

3. 적들의 체력바 구현 및 플레이어 공격 범위 안에 있는 경우에만 표시되도록 구현<br/>
3-1. 영역 밖인데 체력바가 표시되는 버그가 발생, 다음 주차에 원인을 알고 해결하도록 함.<br/>

4. 기존 일반 공격 애니메이션 연결 방식을 개선<br/>
4-1. 해당 작업으로 일반 공격이 끊기는 현상이 조금 조금 완화됨<br/>
...
1. 적들 체력바가 범위를 벗어나도 유지되던 버그 수정<br/>

2. 플레이어 회피 버튼 추가<br/>
2-1. 왼쪽 시프트 키를 눌러서 회피가 가능<br/>
2-2. 피격과 필살기 상태를제외한 모든 상태에서 사용이 가능<br/>
2-3. 회피 상태에서는 무적시간을 갖고 있으며 최대 2번 연속 사용이 가능하고 그 다음에는 쿨타임이 존재<br/>

3. 플레이어 필살기 종류와 모션을 하나 더 추가<br/>
3-1. 필살기 종류는 총 두 개로 둘 중 하나가 랜덤으로 나가는 형식으로 구현<br/>
3-2. 두 개의 필살기는 이펙트와 모션이 서로 다름<br/>

4. 공격 이펙트 및 효과음을 바뀐 일반 공격 방식에 맞게 발생하도록 변경<br/>

5. 적 피격 시스템 방식 변경<br/>
5-1. 피격 스택 시스템을 없애고, 캐릭터의 공격을 맞으면 무조건 피격 애니메이션이 재생되도록 변경<br/>
...
1-2. 플레이어와 거리가 가까워지면 다른 위치로 이동하여 거리를 벌린 다음 다시 화살을 쏨<br/>
...
1-1. 현 플레이어 위치에 마법진을 보여준 다음 일정 시간이 지나면 마법진 구역에 마법 효과가 나와 범위 안에서는 대미지를 지속적으로 입음<br/>

2. 인게임에서 적들이 제대로 동작하는지 확인<br/>

3. 적들의 대미지 수치 및 이동속도, 체력 수치 조정<br/>

4. 각 스테이지의 나오는 적의 종류와 수량 조절<br/>

5. 게임 오버 시 게임 재시작, 타이틀 화면으로 버튼 구현<br/>

6. 적의 공격 방식, 플레이어를 향한 추격 시스템 개선<br/>
6-1. 적이 플레이어에게 다가오면 동작 3종류(가만히 있기, 뒷걸음질, 공격) 이 중 하나를 실행하도록 만듦<br/>
...
1-3. 텐트의 체력은 지속적으로 볼 수 있도록 상단 좌측에 텐트 체력 UI를 추가<br/>
...
1-3. 적이 폭탄에 맞은 경우 대미지를 입고 스턴 상태가 됨<br/>

2. 투척 시 폭발 범위를 알려주는 외관 변경<br/>
2-1. 빨강색 원에서 조준점으로 변경<br/>

3. 인게임 아이템 박스 및 체력바 디자인 변경<br/>
...
1-2. 폰트를 게임 전체적인 컨셉 디자인과 알맞도록 변경<br/>

2. 각종 효과음 추가<br/>

3. 각 적들과 1:1 상황을 만들어 효과음과 애니메이션 싱크 확인 및 버그 수정<br/>
...
1-3. 드래곤이 출현했을 경우 화면 상단에 체력이 표시되도록 UI 수정<br/>
...
1-3. 더욱 멀어질 경우 공중으로 날아서 플레이어에게 다가옴<br/>

2. 파이어볼과 골렘의 바위에 맞았을 경우 피격 이펙트 추가<br/>

3. 드래곤 HP바 UI 디자인 수정<br/>
...
1-2. 설명에 맞는 이미지는 아직 미설정<br/>

2. 타이틀 화면 이미지와 이펙트 추가<br/>

3. 인게임 배경 추가<br/>

4. 골렘 원거리 공격 바위와 폭탄을 던질 때 오브젝트들이 회전하도록 변경<br/>

5. 골렘 원거리 공격에 맞을 경우 일정시간 움직일 수 없는 스턴 추가<br/>

6. 적 드래곤이 나왔을 경우 포효와 함께 카메라 흔들림 효과 추가<br/>

7. 적 드래곤 착지 시 카메라 흔들림 효과 추가<br/>

8. 적 드래곤 착지 공격에 맞을 경우 일정시간 움직일 수 없는 스턴 추가<br/>

9. 적 드래곤 효과음 구현 및 애니메이션에 맞도록 싱크 조절<br/>

10. 각 스테이지 BGM이 다르게 나오도록 구현<br/>

11. 게임 처음 시작 및 각 스테이지 시작 시 로딩 화면이 나오도록 구현<br/>
11-1. 스테이지를 클리어할수록 화면에 캐릭터가 추가됨<br/>
...
1. 플레이어 체력이 0이 될 경우 쓰러지는 애니메이션 추가<br/>

2. 모든 스테이지를 클리어했을 경우 간단한 텍스트와 컷신과 함께 엔딩으로 넘어가도록 구현<br/>

3. 게임 파일 빌드 후 테스트 플레이 및 오류 수정<br/>
3-1. 해당 과정 중 로딩화면에서 로딩이 멈추는 오류 발견, 로딩화면은 삭제<br/>
3-2. 게임을 좀 더 쉽게 즐길 수 있도록 아이템 드랍 확률 조정<br/>
3-3. 게임을 좀 더 쉽게 즐길 수 있도록 적들의 체력 변경<br/>
